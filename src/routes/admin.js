...
router.post('/news', async (req, res) => {
  try {
    ...
    if (!title || !content) return res.status(400).json({ success: false, error: 'Заполните заголовок и текст' });

    const { rows } = await pool.query(
      'INSERT INTO news (title, content, category) VALUES ($1, $2, $3) RETURNING *',
      [title.trim(), content.trim(), (category || 'general').trim()]
    );
...
router.put('/news/:id', async (req, res) => {
  try {
    ...
    if (category !== undefined) { fields.push(`category = $${idx++}`); params.push(category.trim()); }
    if (is_published !== undefined) { fields.push(`is_published = $${idx++}`); params.push(!!is_published); }

    if (!fields.length) return res.status(400).json({ success: false, error: 'Нечего обновлять' });
    ...
    if (!rows.length) return res.status(404).json({ success: false, error: 'Не найдено' });
    res.json({ success: true, item: rows[0] });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
...
router.delete('/news/:id', async (req, res) => {
  try {
    ...
    if (!rowCount) return res.status(404).json({ success: false, error: 'Не найдено' });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
